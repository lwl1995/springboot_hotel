...
<body>
		...
		<table class="tablelist">
			...
			<tbody>
				<c:forEach items="${pageInfo.list }" var="map" varStatus="num">
					<tr>
						<td><input type="checkbox" name="ck" value="${map.id }" /></td>
						<td>${num.count }</td>
						<td>${map.vip_num }</td>
						...
						<td>
							...
							<a href="<%=basePath %>/getVipInfoById.do?id=${map.id }" class="tablelink"> 修改</a>
						</td>
					</tr>
				</c:forEach>
			</tbody>
 ...
 <nav class="text-center">
	...
	$("#add").click(function(){
		window.location.href="<%=basePath %>/addVip.do";
	});
